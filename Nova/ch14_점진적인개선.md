...
사소한 변경이었으나 몇몇 테스트 케이스가 실패하기 시작했다.
getBoolean 함수를 보면, 'y'라는 인수가 없는데 argsfh 'y'를 넘긴다면 booleanArgs.get('y')는 null을 반환하고 함수는 NullPointerException을 던진다. 이전 코드는 falseIfNull 함수로 NullPointerException은 막았으나 내가 고친 코드는 falseIfNull 함수가 더 이상 아무런 효력을 발휘하지 못했다.
